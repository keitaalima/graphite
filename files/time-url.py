...
def time_url(url):
  fetcher = httplib2.Http(".cache")
  t1=time.time()
  resp, content = fetcher.request(url,headers={'cache-control':'no-cache'})
...
def get_timeurl():
  return time_url('http://www.1stdibs.com/furniture/')
...
while True:
  ...
  timeurl = get_timeurl()
  lines.append("mysite.web_responsetime %s %d" % (timeurl,now))
